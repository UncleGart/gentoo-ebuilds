...
multilib_src_configure() {
	...
	local backend="all"
	if [[ "${ABI}" == "x32" ]]; then
		backend="neon"  # DIRTY HACK: orc configure requires at least one backend to be enabled,
		                # however, both MMX and SSE backends fail to work correctly on x32.
	fi
	local emesonargs=(
		-Ddefault_library=$(usex static-libs both shared)
		-Dorc-backend=${backend}
