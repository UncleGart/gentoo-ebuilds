...
IUSE="big-endian clippy cpu_flags_x86_sse2 debug dist doc llvm-libunwind lto miri nightly parallel-compiler rustfmt rust-analyzer rust-src system-llvm system-bootstrap test wasm ${ALL_LLVM_TARGETS[*]}"
...
pkg_setup() {
	...
	if use system-bootstrap; then
		export RUST_SLOT="1.82.0"
		export RUST_SLOT="binary"
		export CARGO="${EPREFIX}/usr/bin/cargo"
		export RUSTC="${EPREFIX}/usr/bin/rustc"
	else
		rust_pkg_setup
	fi
	...
	if use system-llvm; then
		...
		if use abi_x86_x32; then
			llvm_config="$(get_llvm_prefix)/bin/x86_64-pc-linux-gnu-llvm-config"
		fi
...
src_configure() {
	...
	for v in $(multilib_get_enabled_abi_pairs); do
		...
		if use system-llvm; then
			local llvm_config="$(get_llvm_prefix)/bin/llvm-config"
			case ${rust_target} in
				x86_64*gnu)    llvm_config="$(get_llvm_prefix)/bin/x86_64-pc-linux-gnu-llvm-config";;
