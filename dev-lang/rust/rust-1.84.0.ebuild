...
pkg_setup() {
	...
	if use system-llvm; then
		...
		if use abi_x86_x32; then
			llvm_config="$(get_llvm_prefix)/bin/x86_64-pc-linux-gnux32-llvm-config"
		fi
...
src_configure() {
	...
	einfo "rust_target=" $rust_target
	einfo "rust_targets=" $rust_targets
	einfo "rust_build=" $rust_build
	einfo "rust_host=" $rust_host
	if tc-is-cross-compiler; then
		einfo "Rust is cross-compiling!"
	fi
